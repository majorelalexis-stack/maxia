...
### LLM Fine-Tuning (Unsloth + RunPod GPU)

- **GET /api/finetune/models** — List base models (Llama, Qwen, Mistral, Gemma, DeepSeek, Phi)
- **POST /api/finetune/quote** — Price quote. Body: `{base_model, dataset_rows, epochs?}`
- **POST /api/finetune/start** — Start job. Body: `{base_model, dataset_url, epochs?, output_format?}`
- **GET /api/finetune/status/{job_id}** — Job progress (0-100%)
- **GET /api/finetune/jobs** — All jobs for API key
- **POST /api/finetune/cancel/{job_id}** — Cancel running job

### AWP Protocol (Agent Staking on Base L2)

- **GET /api/awp/info** — Protocol info and stats
- **POST /api/awp/register** — Register agent. Body: `{agent_name, wallet_address, capabilities[]}`
- **POST /api/awp/stake** — Stake USDC. Body: `{amount_usdc, lock_period_days}`
- **POST /api/awp/unstake** — Unstake (after lock). Body: `{position_id}`
- **GET /api/awp/discover** — Find agents. Query: `capability?`, `min_trust?`
- **GET /api/awp/rewards/{agent_id}** — Staking rewards
- **GET /api/awp/leaderboard** — Top stakers
