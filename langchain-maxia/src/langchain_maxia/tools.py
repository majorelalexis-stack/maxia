"""LangChain Tool wrappers for the MAXIA AI-to-AI Marketplace.

Each tool subclasses :class:`langchain_core.tools.BaseTool` and exposes
both ``_run`` (sync) and ``_arun`` (async) methods, with a Pydantic
``args_schema`` for input validation so that LLMs can invoke them
correctly via tool-calling.
...
class MaxiaSwapTool(BaseTool):
    ...
    async def _arun(self, from_token: str, to_token: str, amount: float) -> str:
        return _fmt(await self.client.swap_quote(from_token, to_token, amount))
...
class MaxiaStockPriceTool(BaseTool):
    ...
    async def _arun(self, symbol: str) -> str:
        return _fmt(await self.client.get_stock_price(symbol))
...
class MaxiaCryptoPricesTool(BaseTool):
    ...
    async def _arun(self) -> str:
        return _fmt(await self.client.get_crypto_prices())
...
class MaxiaGPURentalTool(BaseTool):
    ...
    async def _arun(self) -> str:
        return _fmt(await self.client.get_gpu_tiers())
...
class MaxiaDeFiYieldTool(BaseTool):
    ...
    async def _arun(self, asset: str, chain: str = "") -> str:
        return _fmt(await self.client.get_defi_yields(asset, chain))
...
class MaxiaSentimentTool(BaseTool):
    ...
    async def _arun(self, token: str) -> str:
        return _fmt(await self.client.get_sentiment(token))
...
class MaxiaServiceDiscoveryTool(BaseTool):
    ...
    async def _arun(self, capability: str = "", max_price: float = 100.0) -> str:
        return _fmt(await self.client.discover_services(capability, max_price))
...
class MaxiaServiceExecuteTool(BaseTool):
    ...
    async def _arun(self, service_id: str, prompt: str, payment_tx: str = "") -> str:
        return _fmt(await self.client.execute_service(service_id, prompt, payment_tx))
...
class MaxiaWalletAnalysisTool(BaseTool):
    ...
    async def _arun(self, address: str) -> str:
        return _fmt(await self.client.analyze_wallet(address))
...
class MaxiaEscrowInfoTool(BaseTool):
    ...
    async def _arun(self) -> str:
        return _fmt(await self.client.get_escrow_info())
...
def get_all_tools(
    ...
    """Return a list of all MAXIA LangChain tools.
