"""Deploy MaxiaEscrow on Polygon mainnet (chain 137).
...
# ══════════════════════════════════════════
# Configuration — Polygon mainnet (chain 137)
# ══════════════════════════════════════════
...
PRIVATE_KEY = os.getenv("POLYGON_DEPLOYER_PRIVKEY", os.getenv("PRIVATE_KEY", ""))
TREASURY_ADDRESS = os.getenv("TREASURY_ADDRESS", "")

if not PRIVATE_KEY:
    print("ERROR: Set POLYGON_DEPLOYER_PRIVKEY or PRIVATE_KEY env var")
    sys.exit(1)
...
print(f"\nDeploying MaxiaEscrow on {CHAIN_NAME} mainnet...")
...
with open(env_path, "w") as f:
    f.write(f"# MaxiaEscrow deployed on {CHAIN_NAME} mainnet\n")
    f.write(f"POLYGON_ESCROW_ADDRESS={contract_address}\n")
    f.write(f"POLYGON_ESCROW_TX={tx_hash.hex()}\n")
    f.write(f"POLYGON_USDC_ADDRESS={USDC_ADDRESS}\n")
    f.write(f"POLYGON_CHAIN_ID={CHAIN_ID}\n")
