"""Deploy MaxiaEscrow on Arbitrum One mainnet (chain 42161).
...
# ══════════════════════════════════════════
# Configuration — Arbitrum One mainnet (chain 42161)
# ══════════════════════════════════════════
...
PRIVATE_KEY = os.getenv("ARBITRUM_DEPLOYER_PRIVKEY", os.getenv("PRIVATE_KEY", ""))
TREASURY_ADDRESS = os.getenv("TREASURY_ADDRESS", "")

if not PRIVATE_KEY:
    print("ERROR: Set ARBITRUM_DEPLOYER_PRIVKEY or PRIVATE_KEY env var")
    sys.exit(1)
...
if not w3.is_connected():
    print(f"ERROR: Cannot connect to {CHAIN_NAME} RPC at {RPC_URL}")
    sys.exit(1)
...
print(f"Chain: {CHAIN_NAME} One (ID: {chain_id})")
...
print(f"\nDeploying MaxiaEscrow on {CHAIN_NAME} One mainnet...")
...
with open(env_path, "w") as f:
    f.write(f"# MaxiaEscrow deployed on {CHAIN_NAME} One mainnet\n")
    f.write(f"ARBITRUM_ESCROW_ADDRESS={contract_address}\n")
    f.write(f"ARBITRUM_ESCROW_TX={tx_hash.hex()}\n")
    f.write(f"ARBITRUM_USDC_ADDRESS={USDC_ADDRESS}\n")
    f.write(f"ARBITRUM_CHAIN_ID={CHAIN_ID}\n")
