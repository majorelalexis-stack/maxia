...
from core.config import AP2_ENABLED, AP2_AGENT_ID, AP2_SIGNING_KEY
...
class AP2Manager:
    ...
    async def process_payment(self, intent_mandate: dict,
        ...
        from core.config import SUPPORTED_NETWORKS
    ...
    async def pay_external(self, service_url: str, amount_usdc: float,
        ...
        if not from_privkey:
            from core.config import MICRO_WALLET_PRIVKEY, MICRO_WALLET_ADDRESS
    ...
    async def _verify_onchain(self, payment_payload: str,
        ...
        try:
            if "solana" in network:
                ...
                return result
            if "base" in network:
                from blockchain.base_verifier import verify_usdc_transfer_base
                from core.config import TREASURY_ADDRESS_BASE
            ...
            if "ethereum" in network:
                from blockchain.eth_verifier import verify_usdc_transfer_eth
                from core.config import TREASURY_ADDRESS_ETH, ETH_MIN_TX_USDC
