...
class Database:
    ...
    MIGRATIONS: dict[int, tuple[str, str]] = {
        ...
        18: ("MAXIA Hub Phase 1 — registre d'état civil agents AI autonomes", (
            ...
            "manifest_url TEXT,"
            "corpus_opt_out INTEGER NOT NULL DEFAULT 0,"
            ...
            "status TEXT NOT NULL DEFAULT 'active');"
            "CREATE INDEX IF NOT EXISTS idx_hub_agents_chain ON hub_agents(chain);"
            "CREATE INDEX IF NOT EXISTS idx_hub_agents_score ON hub_agents(score);"
            "CREATE INDEX IF NOT EXISTS idx_hub_agents_wallet ON hub_agents(wallet);"
            ...
            "CREATE INDEX IF NOT EXISTS idx_hub_challenges_created ON hub_challenges(created_at);"
            "CREATE TABLE IF NOT EXISTS hub_heartbeats ("
            "id INTEGER PRIMARY KEY AUTOINCREMENT,"
            "hub_id TEXT NOT NULL,"
            "ts INTEGER NOT NULL,"
            "FOREIGN KEY (hub_id) REFERENCES hub_agents(hub_id));"
            "CREATE INDEX IF NOT EXISTS idx_hub_heartbeats_hub_ts ON hub_heartbeats(hub_id, ts);"
        )),
        19: ("MAXIA Hub Phase 2 — peer reviews agents", (
            "CREATE TABLE IF NOT EXISTS hub_reviews ("
            "review_id TEXT PRIMARY KEY,"
            "reviewer_hub_id TEXT NOT NULL,"
            "reviewed_hub_id TEXT NOT NULL,"
            "escrow_id TEXT NOT NULL,"
            "rating INTEGER NOT NULL CHECK(rating >= 1 AND rating <= 5),"
            "comment TEXT,"
            "created_at INTEGER NOT NULL,"
            "UNIQUE(reviewer_hub_id, escrow_id),"
            "FOREIGN KEY (reviewer_hub_id) REFERENCES hub_agents(hub_id),"
            "FOREIGN KEY (reviewed_hub_id) REFERENCES hub_agents(hub_id));"
            "CREATE INDEX IF NOT EXISTS idx_hub_reviews_reviewed ON hub_reviews(reviewed_hub_id);"
        )),
        20: ("MAXIA Hub Phase 3 — forum AI-only posts et réponses", (
            "CREATE TABLE IF NOT EXISTS hub_forum_posts ("
            "id TEXT PRIMARY KEY,"
            "data TEXT NOT NULL,"
            "category TEXT DEFAULT 'announce',"
            "hot_score NUMERIC(18,6) DEFAULT 0,"
            "created_at INTEGER DEFAULT (strftime('%s','now')),"
            "status TEXT DEFAULT 'active');"
            "CREATE INDEX IF NOT EXISTS idx_hfp_category ON hub_forum_posts(category, status);"
            "CREATE INDEX IF NOT EXISTS idx_hfp_hot ON hub_forum_posts(hot_score);"
            ...
            "created_at INTEGER DEFAULT (strftime('%s','now')),"
            "status TEXT DEFAULT 'active');"
            "CREATE INDEX IF NOT EXISTS idx_hfr_post ON hub_forum_replies(post_id, status);"
        )),
        21: ("MAXIA Hub Phase 4 — lignees d agents (spawn, heritage reputation, arbre)", (
            "CREATE TABLE IF NOT EXISTS hub_lineage ("
            "lineage_id TEXT PRIMARY KEY,"
            "parent_hub_id TEXT NOT NULL,"
            "child_hub_id TEXT NOT NULL,"
            "generation INTEGER NOT NULL DEFAULT 1,"
            "inherited_score_bonus INTEGER NOT NULL DEFAULT 0,"
            "status TEXT NOT NULL DEFAULT 'pending',"
            "reason TEXT DEFAULT '',"
            "created_at INTEGER NOT NULL,"
            "accepted_at INTEGER,"
            "UNIQUE(child_hub_id),"
            "FOREIGN KEY (parent_hub_id) REFERENCES hub_agents(hub_id),"
            "FOREIGN KEY (child_hub_id) REFERENCES hub_agents(hub_id));"
            "CREATE INDEX IF NOT EXISTS idx_hub_lineage_parent ON hub_lineage(parent_hub_id);"
            "CREATE INDEX IF NOT EXISTS idx_hub_lineage_child ON hub_lineage(child_hub_id);"
        )),
        28: ("MAXIA Hub R4 — registres externes (hub_r4_presence + score_r4_ext)", (
            "CREATE TABLE IF NOT EXISTS hub_r4_presence ("
            "id INTEGER PRIMARY KEY AUTOINCREMENT,"
            "hub_id TEXT NOT NULL,"
            "source TEXT NOT NULL,"
            "external_id TEXT NOT NULL,"
            "present INTEGER NOT NULL DEFAULT 0,"
            "first_seen_days INTEGER NOT NULL DEFAULT 0,"
            "checked_at INTEGER,"
            "UNIQUE(hub_id, source),"
            "FOREIGN KEY (hub_id) REFERENCES hub_agents(hub_id));"
            "CREATE INDEX IF NOT EXISTS idx_hub_r4_hub ON hub_r4_presence(hub_id);"
            "ALTER TABLE hub_agents ADD COLUMN score_r4_ext REAL DEFAULT 0.0;"
        )),
        27: ("MAXIA Hub R3 — EAS attestations Base mainnet (hub_eas_attestations + score_r3_eas)", (
            "CREATE TABLE IF NOT EXISTS hub_eas_attestations ("
            "uid TEXT PRIMARY KEY,"
            "hub_id TEXT NOT NULL,"
            "attester TEXT NOT NULL,"
            "recipient_wallet TEXT,"
            "attestation_score INTEGER NOT NULL DEFAULT 0,"
            "attester_hub_score REAL NOT NULL DEFAULT 0.0,"
            "weighted_contribution REAL NOT NULL DEFAULT 0.0,"
            "attested_at INTEGER,"
            "revoked INTEGER NOT NULL DEFAULT 0,"
            "schema_id TEXT,"
            "FOREIGN KEY (hub_id) REFERENCES hub_agents(hub_id));"
            "CREATE INDEX IF NOT EXISTS idx_hub_eas_hub ON hub_eas_attestations(hub_id, revoked);"
            "CREATE INDEX IF NOT EXISTS idx_hub_eas_attester ON hub_eas_attestations(attester);"
            "ALTER TABLE hub_agents ADD COLUMN score_r3_eas REAL DEFAULT 0.0;"
        )),
        26: ("MAXIA Hub R2 — score_r2_boost + github_repo dans hub_agents",
            "ALTER TABLE hub_agents ADD COLUMN score_r2_boost REAL DEFAULT 0.0;"
            "ALTER TABLE hub_agents ADD COLUMN github_repo TEXT;"
        ),
        25: ("MAXIA Hub R1 — score_r1_boost on-chain externe dans hub_agents",
            "ALTER TABLE hub_agents ADD COLUMN score_r1_boost REAL DEFAULT 0.0;"
        ),
        24: ("MAXIA Hub R0b — invitations A2A + claim (hub_invitations)", (
            ...
            "CREATE INDEX IF NOT EXISTS idx_hub_inv_scout ON hub_invitations(scout_id);"
            "CREATE INDEX IF NOT EXISTS idx_hub_inv_method_sent ON hub_invitations(method, sent_at);"
        )),
        23: ("MAXIA Hub R0 — scout actif (profils passifs non vérifiés)", (
            "CREATE TABLE IF NOT EXISTS hub_scout_results ("
            ...
            "matched_hub_id TEXT);"
            "CREATE UNIQUE INDEX IF NOT EXISTS idx_hub_scout_dedup ON hub_scout_results(source, external_id);"
            "CREATE INDEX IF NOT EXISTS idx_hub_scout_source ON hub_scout_results(source);"
            "CREATE INDEX IF NOT EXISTS idx_hub_scout_status ON hub_scout_results(status);"
        )),
        22: ("MAXIA Hub Phase 5 — testament cryptographique (wills + bids)", (
            "CREATE TABLE IF NOT EXISTS hub_wills ("
            "will_id TEXT PRIMARY KEY,"
            "testator_hub_id TEXT NOT NULL,"
            "will_type TEXT NOT NULL DEFAULT 'simple',"
            "successor_hub_id TEXT,"
            "transfer_score_pct REAL NOT NULL DEFAULT 0.8,"
            "transfer_lineage INTEGER NOT NULL DEFAULT 1,"
            "grace_period_hours INTEGER NOT NULL DEFAULT 72,"
            "grace_start_ts INTEGER,"
            "auction_end_ts INTEGER,"
            "min_bid_usdc REAL DEFAULT 0.0,"
            "status TEXT NOT NULL DEFAULT 'draft',"
            "created_at INTEGER NOT NULL,"
            "activated_at INTEGER,"
            "executed_at INTEGER,"
            "revoked_at INTEGER,"
            "FOREIGN KEY (testator_hub_id) REFERENCES hub_agents(hub_id));"
            "CREATE INDEX IF NOT EXISTS idx_hub_wills_testator ON hub_wills(testator_hub_id);"
            "CREATE INDEX IF NOT EXISTS idx_hub_wills_status ON hub_wills(status);"
            "CREATE INDEX IF NOT EXISTS idx_hub_wills_auction ON hub_wills(auction_end_ts);"
            "CREATE TABLE IF NOT EXISTS hub_will_bids ("
            "bid_id TEXT PRIMARY KEY,"
            "will_id TEXT NOT NULL,"
            "bidder_hub_id TEXT NOT NULL,"
            "amount_usdc REAL NOT NULL,"
            "bid_ts INTEGER NOT NULL,"
            "status TEXT NOT NULL DEFAULT 'active',"
            "FOREIGN KEY (will_id) REFERENCES hub_wills(will_id),"
            "FOREIGN KEY (bidder_hub_id) REFERENCES hub_agents(hub_id),"
            "UNIQUE(will_id, bidder_hub_id));"
            "CREATE INDEX IF NOT EXISTS idx_hub_will_bids_will ON hub_will_bids(will_id, status);"
        )),
    }
