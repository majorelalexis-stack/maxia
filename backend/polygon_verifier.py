"""MAXIA Art.13 — Polygon PoS Verifier & x402 EVM Support
...
_EXPECTED_USDC = "0x3c499c542cEF5E3811e1192ce70d8cC03d5c3359"
if POLYGON_USDC_CONTRACT.lower() != _EXPECTED_USDC.lower():
    logger.critical(
        f"[PolygonVerifier] POLYGON_USDC_CONTRACT mismatch! "
        f"Got {POLYGON_USDC_CONTRACT}, expected {_EXPECTED_USDC}. "
...
if X402_FACILITATOR_URL and not X402_FACILITATOR_URL.startswith("https://"):
    logger.warning(
        f"[PolygonVerifier] X402_FACILITATOR_URL is not HTTPS: {X402_FACILITATOR_URL}. "
...
POLYGON_MIN_TX_USDC = float(os.getenv("POLYGON_MIN_TX_USDC", "0.01"))
...
async def _rpc_post(payload: dict, timeout: float = 20) -> dict:
    """Post to Polygon RPC with fallback across multiple endpoints."""
    ...
    for rpc_url in POLYGON_RPC_URLS:
        try:
            ...
            logger.warning(f"[PolygonVerifier] RPC {rpc_url} timeout: {e}")
            last_error = e
        except httpx.ConnectError as e:
            logger.warning(f"[PolygonVerifier] RPC {rpc_url} connect error: {e}")
            last_error = e
        except Exception as e:
            logger.warning(f"[PolygonVerifier] RPC {rpc_url} unexpected error: {type(e).__name__}: {e}")
            last_error = e
    raise last_error or Exception("All Polygon RPC endpoints failed")


async def verify_polygon_transaction(tx_hash: str, expected_to: str = None) -> dict:
    """Verify a transaction on Polygon PoS via eth_getTransactionReceipt."""
    payload = {
        "jsonrpc": "2.0", "id": 1,
    ...
    for attempt in range(3):
        try:
            ...
            logger.info(f"[PolygonVerifier] TX verified: {tx_hash[:16]}... block={result.get('blockNumber')}")
            return {
                "valid": True,
...
async def verify_usdc_transfer_polygon(tx_hash: str, expected_amount_raw: int = None,
                                        expected_recipient: str = None) -> dict:
    """Verify a USDC ERC-20 Transfer event on Polygon with recipient + amount check."""
    if not expected_recipient:
        if not TREASURY_ADDRESS_POLYGON:
            return {"valid": False, "error": "TREASURY_ADDRESS_POLYGON not configured"}
        expected_recipient = TREASURY_ADDRESS_POLYGON

    if expected_amount_raw is not None and expected_amount_raw > 0:
        min_raw = int(POLYGON_MIN_TX_USDC * 1e6)
        if expected_amount_raw < min_raw:
            return {
                "valid": False,
                "error": f"Amount below minimum: ${expected_amount_raw / 1e6:.4f} < ${POLYGON_MIN_TX_USDC}",
            }

    receipt = await verify_polygon_transaction(tx_hash, expected_to=None)
    ...
    try:
        ...
        for log in logs:
            ...
            if len(topics) < 3:
                continue
            if (log.get("address", "").lower() != POLYGON_USDC_CONTRACT.lower()
                    or topics[0] != transfer_topic):
                continue
            if len(topics[1]) < 42 or len(topics[2]) < 42:
                logger.warning(f"[PolygonVerifier] Malformed topics in tx {tx_hash}: len(topics[1])={len(topics[1])}, len(topics[2])={len(topics[2])}")
                continue
            ...
            logger.info(
                f"[PolygonVerifier] USDC transfer verified: {tx_hash[:16]}... "
...
async def x402_verify_payment_polygon(payment_header: str, expected_amount_usdc: float) -> dict:
    """Verify an x402 payment on Polygon via the facilitator, with direct on-chain fallback."""
    if X402_FACILITATOR_URL and not X402_FACILITATOR_URL.startswith("https://"):
    ...
    if payment_header and payment_header.startswith("0x") and len(payment_header) == 66:
        ...
        try:
            direct_result = await verify_usdc_transfer_polygon(
...
def build_x402_challenge_polygon(path: str, price_usdc: float, pay_to: str) -> dict:
    """Build an x402 402-response payload for Polygon network."""
    return {
        "scheme": "exact",
        "network": "polygon-mainnet",
        ...
        "extra": {"chainId": POLYGON_CHAIN_ID, "facilitator": X402_FACILITATOR_URL},
    }
