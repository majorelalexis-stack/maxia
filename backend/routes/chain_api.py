...
async def xrpl_info():
    """Infos XRP Ledger."""
    from core.config import XRPL_RPC, XRPL_USDC_ISSUER, TREASURY_ADDRESS_XRPL
    return {
...
async def xrpl_verify_usdc(request: Request):
    ...
    if not tx_hash:
        raise HTTPException(400, "tx_hash required")
    try:
        from blockchain.xrpl_verifier import verify_usdc_transfer_xrpl
...
async def ton_info():
    """Infos reseau TON."""
    from core.config import TON_API_URL, TREASURY_ADDRESS_TON, TON_USDT_JETTON
    return {
        "network": "ton-mainnet",
        "api": TON_API_URL,
...
async def sui_info():
    """Infos reseau SUI."""
    from core.config import SUI_RPC, TREASURY_ADDRESS_SUI, SUI_USDC_TYPE
    return {
        "network": "sui-mainnet",
        "rpc": SUI_RPC,
...
async def tron_info():
    """Infos reseau TRON."""
    from core.config import TRON_API_URL, TREASURY_ADDRESS_TRON, TRON_USDT_CONTRACT, TRON_USDC_CONTRACT
    return {
...
async def tron_verify(request: Request):
    ...
    try:
        from blockchain.tron_verifier import verify_tron_transaction
...
async def near_info():
    ...
    from core.config import NEAR_RPC, TREASURY_ADDRESS_NEAR, NEAR_USDC_CONTRACT
    return {
...
async def near_usdc_balance(account_id: str):
    ...
    try:
        from blockchain.near_verifier import get_near_usdc_balance
...
async def aptos_info():
    """Infos reseau Aptos."""
    from core.config import APTOS_API, TREASURY_ADDRESS_APTOS, APTOS_USDC_TYPE
    return {
...
async def aptos_usdc_balance(address: str):
    ...
    try:
        from blockchain.aptos_verifier import get_aptos_usdc_balance
...
async def sei_info():
    """Infos reseau SEI."""
    from core.config import SEI_RPC, SEI_CHAIN_ID, SEI_USDC_CONTRACT, TREASURY_ADDRESS_SEI
    return {
...
async def sei_verify(request: Request):
    ...
    if not tx_hash:
        raise HTTPException(400, "tx_hash required")
    try:
        from blockchain.sei_verifier import verify_sei_transaction
...
async def sei_verify_usdc(request: Request):
    ...
    if not tx_hash:
        raise HTTPException(400, "tx_hash required")
    try:
        from blockchain.sei_verifier import verify_usdc_transfer_sei
...
async def sei_balance(address: str):
    ...
    try:
        from blockchain.sei_verifier import get_sei_balance
