"""MAXIA Art.13 — Avalanche C-Chain Verifier & x402 EVM Support
...
# ── RPC fallback list ──
AVALANCHE_RPC_URLS = [
    os.getenv("AVALANCHE_RPC", "https://api.avax.network/ext/bc/C/rpc"),
    "https://rpc.ankr.com/avalanche",
    "https://avalanche.llamarpc.com",
]

# ── USDC contract assertion at module load ──
_EXPECTED_USDC = "0xB97EF9Ef8734C71904D8002F8b6Bc66Dd9c48a6E"
if AVALANCHE_USDC_CONTRACT.lower() != _EXPECTED_USDC.lower():
    logger.critical(
        f"[AvalancheVerifier] AVALANCHE_USDC_CONTRACT mismatch! "
        f"Got {AVALANCHE_USDC_CONTRACT}, expected {_EXPECTED_USDC}. "
...
if X402_FACILITATOR_URL and not X402_FACILITATOR_URL.startswith("https://"):
    logger.warning(
        f"[AvalancheVerifier] X402_FACILITATOR_URL is not HTTPS: {X402_FACILITATOR_URL}. "
...
AVALANCHE_MIN_TX_USDC = float(os.getenv("AVALANCHE_MIN_TX_USDC", "0.01"))
...
async def _rpc_post(payload: dict, timeout: float = 20) -> dict:
    """Post to Avalanche RPC with fallback across multiple endpoints."""
    await _check_rpc_rate_limit()
    last_error = None
    for rpc_url in AVALANCHE_RPC_URLS:
        try:
            client = get_http_client()
            resp = await client.post(rpc_url, json=payload, timeout=timeout)
            data = resp.json()
            if "error" in data and data["error"]:
                logger.warning(f"[AvalancheVerifier] RPC {rpc_url} returned error: {data['error']}")
                last_error = Exception(f"RPC error: {data['error']}")
                continue
            return data
        except httpx.TimeoutException as e:
            logger.warning(f"[AvalancheVerifier] RPC {rpc_url} timeout: {e}")
            last_error = e
        except httpx.ConnectError as e:
            logger.warning(f"[AvalancheVerifier] RPC {rpc_url} connect error: {e}")
            last_error = e
        except Exception as e:
            logger.warning(f"[AvalancheVerifier] RPC {rpc_url} unexpected error: {type(e).__name__}: {e}")
            last_error = e
    raise last_error or Exception("All Avalanche RPC endpoints failed")


async def verify_avalanche_transaction(tx_hash: str, expected_to: str = None) -> dict:
    """Verify a transaction on Avalanche C-Chain via eth_getTransactionReceipt."""
    payload = {
        "jsonrpc": "2.0", "id": 1,
    ...
    for attempt in range(3):
        try:
            ...
            logger.info(f"[AvalancheVerifier] TX verified: {tx_hash[:16]}... block={result.get('blockNumber')}")
            return {
                "valid": True,
...
async def verify_usdc_transfer_avalanche(tx_hash: str, expected_amount_raw: int = None,
                                          expected_recipient: str = None) -> dict:
    """Verify a USDC ERC-20 Transfer event on Avalanche with recipient + amount check."""
    if not expected_recipient:
        if not TREASURY_ADDRESS_AVALANCHE:
            return {"valid": False, "error": "TREASURY_ADDRESS_AVALANCHE not configured"}
        expected_recipient = TREASURY_ADDRESS_AVALANCHE

    if expected_amount_raw is not None and expected_amount_raw > 0:
        min_raw = int(AVALANCHE_MIN_TX_USDC * 1e6)
        if expected_amount_raw < min_raw:
            return {
                "valid": False,
                "error": f"Amount below minimum: ${expected_amount_raw / 1e6:.4f} < ${AVALANCHE_MIN_TX_USDC}",
            }

    receipt = await verify_avalanche_transaction(tx_hash, expected_to=None)
    ...
    try:
        ...
        for log in logs:
            ...
            if len(topics) < 3:
                continue
            if (log.get("address", "").lower() != AVALANCHE_USDC_CONTRACT.lower()
                    or topics[0] != transfer_topic):
                continue
            if len(topics[1]) < 42 or len(topics[2]) < 42:
                logger.warning(f"[AvalancheVerifier] Malformed topics in tx {tx_hash}: len(topics[1])={len(topics[1])}, len(topics[2])={len(topics[2])}")
                continue
            ...
            logger.info(
                f"[AvalancheVerifier] USDC transfer verified: {tx_hash[:16]}... "
...
async def x402_verify_payment_avalanche(payment_header: str, expected_amount_usdc: float) -> dict:
    """Verify an x402 payment on Avalanche via the facilitator, with direct on-chain fallback."""
    if X402_FACILITATOR_URL and not X402_FACILITATOR_URL.startswith("https://"):
    ...
    try:
        ...
        if resp.status_code == 200 and result.get("valid"):
            logger.info(f"[x402] Avalanche payment verified via facilitator: {result.get('txHash', '')[:16]}...")
            return {
                "valid": True,
    ...
    if payment_header and payment_header.startswith("0x") and len(payment_header) == 66:
        ...
        try:
            direct_result = await verify_usdc_transfer_avalanche(
...
def build_x402_challenge_avalanche(path: str, price_usdc: float, pay_to: str) -> dict:
    """Build an x402 402-response payload for Avalanche network."""
    return {
        "scheme": "exact",
        "network": "avalanche-mainnet",
        ...
        "extra": {"chainId": AVALANCHE_CHAIN_ID, "facilitator": X402_FACILITATOR_URL},
    }
