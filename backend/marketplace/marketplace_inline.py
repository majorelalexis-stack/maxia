...
from core.models import ListingCreateRequest, CommandRequest
...
async def app_store_home():
    ...
    from features.app_store import CATEGORIES, get_featured_agents
...
async def app_store_category(category: str):
    ...
    from features.app_store import get_agents_by_category, CATEGORIES_MAP
    from core.database import db
...
async def app_store_search(q: str = ""):
    ...
    from features.app_store import search_agents
    from core.database import db
    if not q or len(q) < 2:
...
async def forum_create_post_direct(request: Request):
    ...
    from routes.forum import create_post
    from core.database import db
...
async def marketplace_home():
    from features.creator_marketplace import TOOL_CATEGORIES, get_tools, ensure_marketplace_tables
    from core.database import db
...
async def marketplace_category(category: str, sort: str = "popular", limit: int = 20):
    from features.creator_marketplace import get_tools
    from core.database import db
...
async def marketplace_tool_detail(tool_id: str):
    from features.creator_marketplace import get_tool_detail
    from core.database import db
...
async def marketplace_publish(request: Request):
    from features.creator_marketplace import publish_tool
    from core.database import db
...
async def marketplace_purchase(tool_id: str, request: Request):
    ...
    from features.creator_marketplace import purchase_tool
    from core.database import db
    body = await request.json()
    wallet = body.get("wallet", "")
...
async def marketplace_review(tool_id: str, request: Request):
    from features.creator_marketplace import review_tool
    from core.database import db
    body = await request.json()
...
async def marketplace_update(tool_id: str, request: Request):
    from core.security import check_rate_limit
    await check_rate_limit(request)
    from features.creator_marketplace import update_tool_version
    from core.database import db
    body = await request.json()
...
async def marketplace_search(q: str = "", limit: int = 20):
    from features.creator_marketplace import search_tools
    from core.database import db
    return await search_tools(db, q, limit)

@router.get("/api/creator/stats/{wallet}")
async def creator_stats(wallet: str):
    from features.creator_marketplace import get_creator_stats
    from core.database import db
