...
async def get_swap_quote(from_token: str, to_token: str, amount: float,
    ...
    confidence_pct = 0.0
    try:
        from trading.pyth_oracle import CRYPTO_FEEDS, get_pyth_price, get_confidence_threshold, check_twap_deviation
        for sym in [from_token, to_token]:
            feed_id = CRYPTO_FEEDS.get(sym)
            if not feed_id:
                continue
            pyth_data = await get_pyth_price(feed_id, hft=False)
            if "error" in pyth_data:
                continue
        ...
        pass  # Pyth module not available
    except Exception:
    ...
    # P4: Valuation conservative avec confidence bounds Pyth
    # Pour le swap: utiliser prix - confidence pour l'input (worst case buyer)
    # et prix + confidence pour l'output (worst case seller)
    try:
        from trading.pyth_oracle import CRYPTO_FEEDS, get_pyth_price
        for sym in [from_token, to_token]:
            feed_id = CRYPTO_FEEDS.get(sym)
            if not feed_id:
                continue
            pyth_data = await get_pyth_price(feed_id, hft=False)
            conf = pyth_data.get("confidence", 0)
            if conf > 0:
                if sym == from_token:
                    from_price = max(from_price - conf, from_price * 0.99)  # Floor a -1%
                elif sym == to_token:
                    to_price = to_price + conf  # Seller gets slightly less
    except Exception:
        pass
...
async def execute_swap(buyer_api_key: str, buyer_name: str, buyer_wallet: str,
    ...
    try:
        from trading.chainlink_oracle import verify_price_chainlink, CHAINLINK_FEEDS
        for sym in [from_token, to_token]:
    ...
    try:
        from billing.referral import credit_referral_commission
