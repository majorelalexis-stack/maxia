...
from core.config import get_rpc_url, HELIUS_API_KEY
...
async def get_stock_prices() -> dict:
    ...
    try:
        from trading.pyth_oracle import get_pyth_price, EQUITY_FEEDS
        pyth_tasks = []
        pyth_syms = []
        for sym in stocks:
    ...
    if still_missing:
        try:
            from trading.pyth_oracle import get_stock_price_finnhub
