...
async def get_stock_price(symbol: str) -> dict:
    ...
    try:
        from trading.price_oracle import get_price as cg_get_price
    ...
    try:
        from trading.price_oracle import get_stock_prices as yahoo_get_stocks
...
async def get_batch_prices(symbols: list[str]) -> dict:
    ...
    if len(fallback_symbols) > _MAX_FALLBACK:
        ...
        from trading.price_oracle import FALLBACK_PRICES as _fb_prices
        for sym in skipped:
            if sym not in results:
                results[sym] = {
    ...
    if fallback_symbols:
        try:
            from trading.price_oracle import get_prices as cg_get_prices, FALLBACK_PRICES
...
async def _fallback_refresh_loop():
    ...
    while True:
        try:
            ...
            from trading.price_oracle import get_crypto_prices, FALLBACK_PRICES
