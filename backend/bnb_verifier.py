"""MAXIA Art.13 — BNB Chain (BSC) Verifier & x402 EVM Support
Production-hardened: RPC fallback, rate limiting, proper logging, min amount checks.
"""
import os, asyncio, time, logging
import httpx
from config import (
    BNB_RPC, BNB_CHAIN_ID, BNB_USDC_CONTRACT,
    X402_FACILITATOR_URL, TREASURY_ADDRESS_BNB,
)

logger = logging.getLogger("maxia.bnb_verifier")

# ── RPC fallback list ──
BNB_RPC_URLS = [
    os.getenv("BNB_RPC", "https://bsc-dataseed.binance.org"),
    "https://rpc.ankr.com/bsc",
    "https://bsc.llamarpc.com",
]

# ── USDC contract assertion at module load ──
_EXPECTED_USDC = "0x8AC76a51cc950d9822D68b83fE1Ad97B32Cd580d"
if BNB_USDC_CONTRACT.lower() != _EXPECTED_USDC.lower():
    logger.critical(
        f"[BNBVerifier] BNB_USDC_CONTRACT mismatch! "
        f"Got {BNB_USDC_CONTRACT}, expected {_EXPECTED_USDC}. "
...
if X402_FACILITATOR_URL and not X402_FACILITATOR_URL.startswith("https://"):
    logger.warning(
        f"[BNBVerifier] X402_FACILITATOR_URL is not HTTPS: {X402_FACILITATOR_URL}. "
...
BNB_MIN_TX_USDC = float(os.getenv("BNB_MIN_TX_USDC", "0.01"))
...
async def _rpc_post(payload: dict, timeout: float = 20) -> dict:
    """Post to BNB Chain RPC with fallback across multiple endpoints."""
    await _check_rpc_rate_limit()
    last_error = None
    for rpc_url in BNB_RPC_URLS:
        try:
            async with httpx.AsyncClient(timeout=timeout) as client:
                resp = await client.post(rpc_url, json=payload)
                data = resp.json()
            if "error" in data and data["error"]:
                logger.warning(f"[BNBVerifier] RPC {rpc_url} returned error: {data['error']}")
                last_error = Exception(f"RPC error: {data['error']}")
                continue
            return data
        except httpx.TimeoutException as e:
            logger.warning(f"[BNBVerifier] RPC {rpc_url} timeout: {e}")
            last_error = e
        except httpx.ConnectError as e:
            logger.warning(f"[BNBVerifier] RPC {rpc_url} connect error: {e}")
            last_error = e
        except Exception as e:
            logger.warning(f"[BNBVerifier] RPC {rpc_url} unexpected error: {type(e).__name__}: {e}")
            last_error = e
    raise last_error or Exception("All BNB Chain RPC endpoints failed")


async def verify_bnb_transaction(tx_hash: str, expected_to: str = None) -> dict:
    """Verify a transaction on BNB Chain via eth_getTransactionReceipt."""
    payload = {
        "jsonrpc": "2.0", "id": 1,
    ...
    for attempt in range(3):
        try:
            ...
            logger.info(f"[BNBVerifier] TX verified: {tx_hash[:16]}... block={result.get('blockNumber')}")
            return {
                "valid": True,
            ...
            logger.warning(f"[BNBVerifier] verify_bnb_transaction attempt {attempt + 1} timeout: {e}")
            await asyncio.sleep(2 ** attempt)
        except httpx.ConnectError as e:
            logger.warning(f"[BNBVerifier] verify_bnb_transaction attempt {attempt + 1} connect error: {e}")
            await asyncio.sleep(2 ** attempt)
        except Exception as e:
            logger.error(f"[BNBVerifier] verify_bnb_transaction attempt {attempt + 1} failed: {type(e).__name__}: {e}")
            await asyncio.sleep(2 ** attempt)
    return {"valid": False, "error": "Verification failed after retries"}


async def verify_usdc_transfer_bnb(tx_hash: str, expected_amount_raw: int = None,
                                    expected_recipient: str = None) -> dict:
    """Verify a USDC ERC-20 Transfer event on BNB Chain with recipient + amount check."""
    if not expected_recipient:
        if not TREASURY_ADDRESS_BNB:
            return {"valid": False, "error": "TREASURY_ADDRESS_BNB not configured"}
        expected_recipient = TREASURY_ADDRESS_BNB

    if expected_amount_raw is not None and expected_amount_raw > 0:
        min_raw = int(BNB_MIN_TX_USDC * 1e6)
        if expected_amount_raw < min_raw:
            return {
                "valid": False,
                "error": f"Amount below minimum: ${expected_amount_raw / 1e6:.4f} < ${BNB_MIN_TX_USDC}",
            }

    receipt = await verify_bnb_transaction(tx_hash, expected_to=None)
    ...
    try:
        ...
        for log in logs:
            ...
            if len(topics) < 3:
                continue
            if (log.get("address", "").lower() != BNB_USDC_CONTRACT.lower()
                    or topics[0] != transfer_topic):
                continue
            if len(topics[1]) < 42 or len(topics[2]) < 42:
                logger.warning(f"[BNBVerifier] Malformed topics in tx {tx_hash}: len(topics[1])={len(topics[1])}, len(topics[2])={len(topics[2])}")
                continue
            ...
            logger.info(
                f"[BNBVerifier] USDC transfer verified: {tx_hash[:16]}... "
        ...
        logger.error(f"[BNBVerifier] verify_usdc_transfer_bnb timeout: {e}")
        return {"valid": False, "error": f"RPC timeout: {e}"}
    except httpx.ConnectError as e:
        logger.error(f"[BNBVerifier] verify_usdc_transfer_bnb connect error: {e}")
        return {"valid": False, "error": f"RPC connection error: {e}"}
    except Exception as e:
        logger.error(f"[BNBVerifier] verify_usdc_transfer_bnb error: {type(e).__name__}: {e}")
        return {"valid": False, "error": str(e)}


async def x402_verify_payment_bnb(payment_header: str, expected_amount_usdc: float) -> dict:
    """Verify an x402 payment on BNB Chain via the facilitator, with direct on-chain fallback."""
    if X402_FACILITATOR_URL and not X402_FACILITATOR_URL.startswith("https://"):
    ...
    try:
        ...
        if resp.status_code == 200 and result.get("valid"):
            logger.info(f"[x402] BNB payment verified via facilitator: {result.get('txHash', '')[:16]}...")
            return {
                "valid": True,
    ...
    if payment_header and payment_header.startswith("0x") and len(payment_header) == 66:
        ...
        try:
            direct_result = await verify_usdc_transfer_bnb(
...
def build_x402_challenge_bnb(path: str, price_usdc: float, pay_to: str) -> dict:
    """Build an x402 402-response payload for BNB Chain network."""
    return {
        "scheme": "exact",
        "network": "bnb-mainnet",
        ...
        "extra": {"chainId": BNB_CHAIN_ID, "facilitator": X402_FACILITATOR_URL},
    }
