...
async def ceo_full_state(request: Request):
    ...
    try:
        from agents.ceo_maxia import ceo, get_llm_costs
...
async def ceo_execute_action(request: Request):
    ...
    from core.auth import require_ceo_auth
    await require_ceo_auth(request, request.headers.get("X-CEO-Key"))
    from core.security import check_ceo_spending_limit, record_ceo_action, audit_log
    ...
    if action == "scout_contact":
        try:
            from agents.ceo_maxia import ceo, scout_first_contact_a2a
    ...
    if action == "scout_scan":
        try:
            from agents.ceo_maxia import ceo, scout_scan_onchain_agents
...
async def ceo_approval_result(action_id: str, request: Request):
    ...
    from core.auth import require_ceo_auth
    await require_ceo_auth(request, request.headers.get("X-CEO-Key"))
    from integrations.telegram_bot import get_approval_result
...
async def ceo_think(request: Request):
    ...
    try:
        ...
        from agents.ceo_maxia import CEO_IDENTITY
...
async def ceo_llm_costs(request: Request):
    ...
    from core.security import require_admin
    require_admin(request)
    from agents.ceo_maxia import get_llm_costs
...
async def ceo_twitter_analytics(request: Request):
    ...
    conv_path = Path(__file__).parent.parent.parent / "local_ceo" / "conversions.json"
    if conv_path.exists():
        try:
    ...
    scores_path = Path(__file__).parent.parent.parent / "local_ceo" / "platform_scores.json"
    ...
    actions_path = Path(__file__).parent.parent.parent / "local_ceo" / "actions_today.json"
    ...
    learn_path = Path(__file__).parent.parent.parent / "local_ceo" / "learnings.json"
...
async def ceo_get_objectives(request: Request):
    ...
    from core.auth import require_ceo_auth
    await require_ceo_auth(request, request.headers.get("X-CEO-Key"))
    from agents.ceo_objectives import get_objectives, check_pivot_needed
    data = get_objectives()
...
async def ceo_set_objective(request: Request):
    ...
    await require_ceo_auth(request, request.headers.get("X-CEO-Key"))
    body = await request.json()
    from agents.ceo_objectives import set_weekly_objective
...
async def ceo_update_progress(request: Request):
    ...
    await require_ceo_auth(request, request.headers.get("X-CEO-Key"))
    body = await request.json()
    from agents.ceo_objectives import update_progress
